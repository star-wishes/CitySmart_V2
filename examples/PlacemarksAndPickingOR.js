...
requirejs(['./WorldWindShim',
    ...
    function (WorldWind,
        ...
        var newLibrary = WorldWind.configuration.baseUrl + "images/pushpins/", // location of the image files
            ...
            newmarkLayer = new WorldWind.RenderableLayer("New Placemarks"),
            newlatitude = 50,
            newlongitude = -120;
        ...
        // For each pinmark image, create a bowmark with a label.
        for (var j = 0, lent = newimages.length; j < lent; j++) {
            // Create the pinmark and its label.
            newmark = new WorldWind.Placemark(new WorldWind.Position(newlatitude, newlongitude + j, 1e2), true, null);
            newmark.label = "Bluemark" + j.toString() + "\n"
            ...
            // Create the newmark attributes for this bowmark. Note that the attributes differ only by their
            // image URL.
            newmarkAttributes = new WorldWind.PlacemarkAttributes(newmarkAttributes);
            newmarkAttributes.imageSource = newLibrary + newimages[j];
            newmark.attributes = newmarkAttributes;

            // Create the highlight attributes for this newmark. Note that the normal attributes are specified as
            // the default highlight attributes so that all properties are identical except the image scale. You could
            // instead vary the color, image, or other property to control the highlight representation.
            newhighlightAttributes = new WorldWind.PlacemarkAttributes(newmarkAttributes);
            newhighlightAttributes.imageScale = 1.2;
            newmark.highlightAttributes = newhighlightAttributes;

            // Add the newmark to the layer.
            newmarkLayer.addRenderable(newmark);
        }

        // Add the newmarks layer to the WorldWindow's layer list.
